...
export class WarmupService {
    ...
    private startWarmup() {
        ...
        if (isStandalone) {
            console.log(
                '[Warmup] Running as installed PWA (standalone mode). Disabling heartbeat warmup ping.',
            );
            return;
        }

        console.log(
            '[Warmup] Running in browser. Initializing heartbeat warmup ping (every 30s)...',
        );
        ...
        timer(0, 30000)
            .pipe(
                ...
                catchError((err) => {
                    ...
                    console.warn(
                        'Warmup ping failed (expected if server is sleeping or offline):',
                        err.message,
                    );
                    return of(null);
                }),
